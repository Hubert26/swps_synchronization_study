...
class Data:
    ...
    def trim(self, new_start, new_end):
        ...
        if new_start >= new_end:
            raise ValueError(
                f"Invalid range: new_start ({new_start}) must be less than new_end ({new_end})."
            )

        # 1. Select the indices where x_data is within the specified range [new_start, new_end]
        selected_indices = np.where(
            (self.x_data >= new_start) & (self.x_data <= new_end)
        )[0]

        # Check if any indices are selected
        if selected_indices.size == 0:
            raise ValueError(
                f"No data points found in the range [{new_start}, {new_end}]."
            )
        ...
        if len(trimmed_x) == 0 or len(trimmed_y) == 0:
            raise ValueError(
                f"Trimming resulted in empty data. Check the provided range [{new_start}, {new_end}]."
            )
        if len(trimmed_x) != len(trimmed_y):
            raise ValueError(
                "Mismatch between the lengths of trimmed x_data and y_data."
            )
...
class Meas:
    ...
    def __init__(
        ...
        self.metadata = Metadata(
            meas_number, condition, gender, pair_number, shift, starttime, endtime
        )
    ...
    def __add__(self, other: "Meas") -> "Meas":
        ...
        if (
            self.metadata.meas_number != other.metadata.meas_number
            or self.metadata.condition != other.metadata.condition
            or self.metadata.gender != other.metadata.gender
            or self.metadata.pair_number != other.metadata.pair_number
            or self.metadata.shift != other.metadata.shift
        ):
            raise ValueError(
                "Cannot merge Meas objects with different metadata attributes."
            )

        # Ensure the current Meas is older
        if self.metadata.endtime > other.metadata.starttime:
            raise ValueError(
                "Cannot merge: other Meas must start after the current Meas ends."
            )

        # Calculate time difference in milliseconds
        diff_starttime = (
            other.metadata.starttime - self.metadata.starttime
        ).total_seconds() * 1000

        # Concatenate y_data and adjust x_data by the time shift
        new_y_data = np.concatenate((self.data.y_data, other.data.y_data))
        new_x_data = np.concatenate(
            (self.data.x_data, other.data.x_data + diff_starttime)
        )
    ...
    def shift_right(self, shift_ms: float):
        ...
        self.metadata.update(
            starttime=new_starttime, endtime=new_endtime, shift=new_shift
        )
...
class MeasurementRecord:
    ...
    def to_dict(self) -> dict:
        ...
        """
        record_dict = attr.asdict(
            self, recurse=False
        )  # Get dictionary without recursion
