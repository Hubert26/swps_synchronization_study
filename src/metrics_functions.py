...
from utils.signal_utils import (
    filter_values_by_sd,
    filter_values_by_relative_mean,
...
def filter_rr_meas(
    ...
    assert all(isinstance(meas, Meas) for meas in meas_list), (
        "meas_list contains non-Meas objects."
    )
    ...
    for meas in meas_list_copy:
        ...
        try:
            filtered_y_data = filter_values_by_relative_mean(
                filtered_y_data, threshold_factor
            )
        except ValueError as e:
            log_message(
                f"Skipping relative mean filter for {meas} due to: {e}", logging.WARNING
            )
            continue

        # Step 3: Interpolate missing (NaN) values in the filtered data
        filtered_y_data = interpolate_missing_values(
            filtered_y_data, method=interp_method
        )
        ...
        meas.data.update(x_data=new_x_data, y_data=filtered_y_data)
...
# %%
def calculate_overlapping_sd(
    meas: Meas, window_ms: float, overlap: float, min_fraction: float
) -> None:
    """
    ...
    new_y_data, new_x_data = overlapping_sd(
        (time, values),
        window_time=window_ms,
        overlap=overlap,
    ...
    meas.data.update(x_data=new_x_data, y_data=new_y_data)

    meas.metadata.update(endtime=new_endtime)


# %%
def calculate_overlapping_sd_meas(
    meas_list: list[Meas], window_ms: float, overlap: float, min_fraction: float
) -> list[Meas]:
    """
    ...
    for meas in sd_meas_list:
        ...
        calculate_overlapping_sd(
            meas, window_ms=window_ms, overlap=overlap, min_fraction=min_fraction
        )
...
# %%
def calculate_overlapping_rmssd(
    meas: Meas, window_ms: float, overlap: float, min_fraction: float
) -> None:
    """
    ...
    new_y_data, new_x_data = overlapping_rmssd(
        (time, values),
        window_time=window_ms,
        overlap=overlap,
        min_fraction=min_fraction,
...
# %%
def calculate_overlapping_rmssd_meas(
    meas_list: list[Meas], window_ms: float, overlap: float, min_fraction: float
) -> list[Meas]:
    """
