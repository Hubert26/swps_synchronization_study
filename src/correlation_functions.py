...
def calc_corr_weighted(meas_pair_lists: tuple[list[Meas], list[Meas]]) -> tuple:
    ...
    for meas1 in meas1_list:
        for meas2 in meas2_list:
            ...
            if interp_meas1 is None or interp_meas2 is None:
                logger.info(
                    f"Skipping interpolation due to failure. For: {meas1}, {meas2}"
                )
                continue
            ...
            # Calculate the spearman correlation coefficient and p-value between the interpolated signals
            corr, p_val = spearmanr(interp_meas1.data.y_data, interp_meas2.data.y_data)

            # Use fisher transformation on spearman correlation result
    ...
    if not corr_list:
        logger.info(
            f"No valid correlations were computed for {meas1_list} and {meas2_list}"
        )
        return (
            None,
            None,
            meas1.metadata.shift - meas2.metadata.shift,
            str(meas1),
            str(meas2),
            None,
            None,
    ...
    try:
        ...
        for meas in interp_meas1_list:
            logger.warning(
                f"Meas: {str(meas)}, Start time: {meas.metadata.starttime}, End time: {meas.metadata.endtime}"
            )
    ...
    combined_p_val = 1 - chi2.cdf(
        fisher_stat, 2 * len(p_val_list)
...
def process_meas_and_find_corr(meas_list: list[Meas]) -> list[MeasurementRecord]:
    ...
    for key, group in grouped_meas.items():
        ...
        for (start_ms, end_ms), task in selected_intervals.items():
            ...
            selected_shifted_meas1_list = [
                meas
                for meas in shifted_meas1_list
                if lower_bound <= meas.metadata.endtime
                and meas.metadata.starttime <= upper_bound
            ]
            selected_shifted_meas2_list = [
                meas
            ...
            for loop_meas1 in selected_person_meas1:
                try:
                    tm1 = trim_meas(
                        loop_meas1, start_ms, end_ms, starttime=oldest_starttime
                    )
                except ValueError as e:
                    logger.info(
                        f"{meas_number}, {condition}, {pair_number}, {task}: Trimming meas to interval faild. Skipping meas due to invalid time range: {e}"
                    )
                    continue
            ...
            for loop_meas2 in selected_person_meas2:
                try:
                    tm2 = trim_meas(
                        loop_meas2, start_ms, end_ms, starttime=oldest_starttime
                    )
                except ValueError as e:
                    logger.info(
                        f"{meas_number}, {condition}, {pair_number}, {task}: Trimming meas to interval faild. Skipping meas due to invalid time range: {e}"
            ...
            for loop_meas1 in selected_shifted_meas1_list:
                try:
                    ...
                    logger.info(
                        f"{meas_number}, {condition}, {pair_number}, {task}: Trimming shifted meas to interval faild. Skipping shifted meas due to invalid time range: {e}"
                    )
                    continue
            ...
            if not trimmed_meas1_list and not trimmed_shifted_meas1_list:
                logger.warning(
                    f"{meas_number}, {condition}, {pair_number}, {task}: Skipping pair. Lack of trimmed meas1"
                )
                continue

            if not trimmed_meas2_list and not trimmed_shifted_meas2_list:
                logger.warning(
                    f"{meas_number}, {condition}, {pair_number}, {task}: Skipping pair. Lack of trimmed meas2"
                )
                continue
            ...
            if trimmed_meas1_list and trimmed_meas2_list:
                (
                    ...
                    interp_meas1,
                    interp_meas2,
                ) = calc_corr_weighted((trimmed_meas1_list, trimmed_meas2_list))
                if (
                    corr is not None
                    and interp_meas1 is not None
                    and interp_meas2 is not None
                ):
                    # Ensure that the duration of each interpolated measurement exceeds MIN_DURATION_RATIO * interval_duration_min
                    if (
                        interp_meas1.metadata.duration_min
                        > interval_duration_min * MIN_DURATION_RATIO
                        and interp_meas2.metadata.duration_min
                        > interval_duration_min * MIN_DURATION_RATIO
                    ):
            ...
            if trimmed_shifted_meas1_list and trimmed_meas2_list:
                trimmed_shifted_gruped_meas1 = group_object_list(
                    trimmed_shifted_meas1_list, ["metadata.shift"]
                )
                ...
                for shift1, meas1_group in trimmed_shifted_gruped_meas1.items():
                    (
                        ...
                        interp_meas1,
                        interp_meas2,
                    ) = calc_corr_weighted((meas1_group, trimmed_meas2_list))
                    if (
                        corr is not None
                        and interp_meas1 is not None
                        and interp_meas2 is not None
            ...
            if trimmed_shifted_meas2_list and trimmed_meas1_list:
                trimmed_shifted_gruped_meas2 = group_object_list(
                    trimmed_shifted_meas2_list, ["metadata.shift"]
                )
                ...
                for shift2, meas2_group in trimmed_shifted_gruped_meas2.items():
                    (
                        ...
                        interp_meas1,
                        interp_meas2,
                    ) = calc_corr_weighted((trimmed_meas1_list, meas2_group))
                    if (
                        corr is not None
                        and interp_meas1 is not None
                        and interp_meas2 is not None
